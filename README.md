...
### Instanced sub-objects (2.6.2)

Property writes in `DataAssetImport`, `CreateAsset`, `EditBlueprint`, `EditAnimAsset`, `EditDataTable` and `EditPCGGraph` take `{"Class": ..., "Properties": {...}}` for an instanced property, and an array of those for an instanced array. With `Class` a fresh instance is built with the same flags as the Details panel class picker and saves with the asset, without it the existing instance is edited.

Syntax: `Docs/Import.md`, under `DataAssetImport`
